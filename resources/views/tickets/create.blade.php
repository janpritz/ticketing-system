                    ...
                    <div class="mb-6">
                        <div class="bg-blue-50 border-l-4 border-blue-400 p-4">
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-blue-800">Data Privacy Notice</h3>
                                    <div class="mt-2 text-sm text-blue-700">
                                        <p>By proceeding with ticket creation, you acknowledge that all data provided will be used solely for ticket creation and support purposes. We are committed to protecting your privacy and will not share your information with third parties.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- Email Collection Disclaimer -->
                    <div class="mb-6">
                        <div class="bg-blue-50 border-l-4 border-blue-400 p-4">
                            <div class="flex">
                                <div class="flex-shrink-0">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-blue-800">Why we need to collect your email?</h3>
                                    <div class="mt-2 text-sm text-blue-700">
                                        <p>Your email address is essential for communication and support. We use it to send you ticket confirmation, deliver our response, and ensure all updates are received by the correct person. We'll first verify your email by sending a One-Time Password (OTP).</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="email-form" class="space-y-4">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="privacy_checkbox" name="privacy_checkbox" type="checkbox"
                                    class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                            </div>
                            <div class="ml-3 text-sm">
                                <label for="privacy_checkbox" class="font-medium text-gray-700">
                                    I have read and agree to the privacy conditions
                                </label>
                                <p class="text-gray-500">Your data will be used only for ticket creation and support purposes.</p>
                            </div>
                        </div>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            ...
                            document.getElementById('verification_email').addEventListener('input', function() {
                                ...
                                sendButton.disabled = !email || !email.includes('@') || !checkbox.checked;
                            });

                            document.getElementById('privacy_checkbox').addEventListener('change', function() {
                                const email = document.getElementById('verification_email').value;
                                const sendButton = document.getElementById('send-otp-btn');
                                
                                sendButton.disabled = !email || !email.includes('@') || !this.checked;
                            });
                            ...
                            document.getElementById('verify-otp-btn').addEventListener('click', function() {
                                ...
                                fetch('{{ route("tickets.verify-otp") }}', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                                ...
                                .then(data => {
                                    ...
                                    if (data.success) {
                                        ...
                                        Swal.fire({
                                            icon: 'success',
                                            title: 'Success',
                                            text: 'Email verified successfully! Redirecting to ticket submission...',
                                            timer: 2000,
                                            timerProgressBar: true,
                                            showConfirmButton: false
                                        }).then(() => {
                                            // Redirect to ticket submission page
                                            const recipientId = '{{ $recepient_id ?? "" }}';
                                            window.location.href = `/tickets/submit/${recipientId}/${encodeURIComponent(email)}`;
                                        });
                                    } else {
                                        Swal.fire({
                                            icon: 'error',

...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (openTermsBtn) {
            openTermsBtn.addEventListener('click', () => showModal(termsModal));
        }

        // Close Privacy Modal
        document.querySelectorAll('[data-close-policy]').forEach(el => {
            el.addEventListener('click', () => hideModal(privacyModal));
        });

        // Close Terms Modal
        document.querySelectorAll('[data-close-terms]').forEach(el => {
            el.addEventListener('click', () => hideModal(termsModal));
        });

        // Close on Escape key
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                if (!privacyModal.classList.contains('hidden')) hideModal(privacyModal);
                if (!termsModal.classList.contains('hidden')) hideModal(termsModal);
            }
        });

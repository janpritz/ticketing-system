...
@section('staff-scripts')
    ...
    <style>
        /* Spinner Animation for Response Button */
        .btn-spinner {
            ...
            <div class="p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div class="flex-1 max-w-md">
                    <div class="relative">
                        ...
                        <input type="text" id="searchInput" placeholder="Search tickets by ID, subject, or email..."
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button class="group inline-flex items-center gap-1" data-sort="subject">
                                    Subject
                                    <svg class="w-4 h-4 text-gray-400 group-hover:text-gray-600 sort-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M5 12a1 1 0 102 0V6.414l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414L5 6.414V12zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z"/>
                                    </svg>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button class="group inline-flex items-center gap-1" data-sort="assignee">
                                    Assignee
                                    <svg class="w-4 h-4 text-gray-400 group-hover:text-gray-600 sort-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M5 12a1 1 0 102 0V6.414l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414L5 6.414V12zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z"/>
                                    </svg>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button class="group inline-flex items-center gap-1" data-sort="created">
                                    Created Date
                                    <svg class="w-4 h-4 text-gray-400 group-hover:text-gray-600 sort-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M5 12a1 1 0 102 0V6.414l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414L5 6.414V12zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z"/>
                                    </svg>
    ...
    <script>
        ...
        const elements = {
            ...
            counts: {
                ...
                forwarded: document.getElementById('count-forwarded'),
        ...
        function initializeEventListeners() {
            ...
            // Pagination
            if (elements.pagination.prevBtn) {
                elements.pagination.prevBtn.addEventListener('click', () => changePage(currentPage - 1));
            }
            if (elements.pagination.nextBtn) {
                elements.pagination.nextBtn.addEventListener('click', () => changePage(currentPage + 1));
            }
            if (elements.pagination.mobilePrevBtn) {
                elements.pagination.mobilePrevBtn.addEventListener('click', () => changePage(currentPage - 1));
            }
            if (elements.pagination.mobileNextBtn) {
                elements.pagination.mobileNextBtn.addEventListener('click', () => changePage(currentPage + 1));
            }
        }
        ...
        async function loadTickets() {
            try {
                ...
                const url = '/staff/tickets/data';
                console.log('Loading tickets from URL:', url);
                ...
                console.log('Tickets response headers:', [...response.headers.entries()]);

                if (!response.ok) {
                    console.error('Failed to load tickets', response.status, response.statusText);
                    
                    // Get error response text for debugging
                    let errorText = '';
                    try {
                        errorText = await response.text();
                        console.log('Tickets error response text:', errorText.substring(0, 500));
                    } catch (e) {
                ...
                console.log('Tickets response Content-Type:', contentType);
                ...
                if (!contentType || !contentType.includes('application/json')) {
                    ...
                    try {
                        ...
                        console.log('Actual tickets response (first 500 chars):', responseText.substring(0, 500));
                    } catch (e) {
                        console.log('Could not read response as text');
                    }
                    
                    throw new Error('Server returned an unexpected response format instead of JSON');
                }

                const data = await response.json();
                console.log('Parsed tickets data:', data);
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500">Failed to load tickets. Please try again.</p>
        ...
        function updateCounts() {
            ...
            const counts = {
                ...
                forwarded: ticketsData.filter(t => t.status === 'Forwarded').length,
                closed: ticketsData.filter(t => t.status === 'Closed').length,
            };

            Object.keys(counts).forEach(key => {
                if (elements.counts[key]) {
                    elements.counts[key].textContent = counts[key];
                }
            });
        }
        ...
        function setActiveFilter(filter) {
            ...
            if (elements.tableTitle) {
                elements.tableTitle.textContent = titles[filter] || 'Tickets';
            }
        ...
        function applyFiltersAndRender() {
            ...
            filteredTickets = ticketsData.filter(ticket => {
                // Status filter
                if (currentFilter !== 'all' && ticket.status.toLowerCase() !== currentFilter) {
                    return false;
                }
                ...
                if (searchTerm) {
                    ...
                    return (
                        ...
                        (ticket.question && ticket.question.toLowerCase().includes(searchLower)) ||
            ...
            filteredTickets.sort((a, b) => {
                ...
                switch (sortField) {
                    ...
                    case 'subject':
                        ...
                        break;
                    case 'assignee':
                        aVal = (a.staff && a.staff.name) || '';
                        bVal = (b.staff && b.staff.name) || '';
                        break;
                    case 'created':
                        aVal = new Date(a.date_created || a.created_at);
                        bVal = new Date(b.date_created || b.created_at);
                        break;
                    default:
                        aVal = new Date(a.date_created || a.created_at);
                        bVal = new Date(b.date_created || b.created_at);
        ...
        function renderTickets() {
            ...
            const endIndex = Math.min(startIndex + perPage, filteredTickets.length);
            const pageTickets = filteredTickets.slice(startIndex, endIndex);

            const rows = pageTickets.map(ticket => {
                const statusStyle = statusStyles[ticket.status] || 'text-slate-700 bg-slate-50 ring-slate-600/20';
                const assignee = (ticket.staff && ticket.staff.name) ? ticket.staff.name : 'Unassigned';
                const createdDate = formatDate(ticket.date_created || ticket.created_at);
                const updatedDate = formatDate(ticket.updated_at);
                const subject = ticket.question ? (ticket.question.length > 60 ? ticket.question.substring(0, 57) + '...' : ticket.question) : 'No subject';

                return `
                    <tr class="hover:bg-gray-50 cursor-pointer transition-colors" data-id="${ticket.id}">
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-indigo-600">#${ticket.id}</div>
                            <div class="text-xs text-gray-500">${createdDate}</div>
                        </td>
                        <td class="px-4 py-4">
                            <div class="text-sm text-gray-900 max-w-xs truncate" title="${ticket.question || ''}">${subject}</div>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ring-1 ${statusStyle}">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">${assignee}</div>
                            <div class="text-xs text-gray-500">Updated ${updatedDate}</div>
                        </td>
            ...
            elements.ticketsTableBody.innerHTML = rows;
        ...
        function renderPagination() {
            ...
            if (totalPages <= 1) {
                updatePaginationInfo(0, 0, filteredTickets.length);
                return;
            }

            const startIndex = (currentPage - 1) * perPage + 1;
            const endIndex = Math.min(currentPage * perPage, filteredTickets.length);
            
            updatePaginationInfo(startIndex, endIndex, filteredTickets.length);

            // Update navigation buttons
            if (elements.pagination.prevBtn) {
                elements.pagination.prevBtn.disabled = currentPage === 1;
            }
            if (elements.pagination.nextBtn) {
                elements.pagination.nextBtn.disabled = currentPage === totalPages;
            }
            if (elements.pagination.mobilePrevBtn) {
                elements.pagination.mobilePrevBtn.disabled = currentPage === 1;
            }
            if (elements.pagination.mobileNextBtn) {
                elements.pagination.mobileNextBtn.disabled = currentPage === totalPages;
            }
        ...
        function renderPageNumbers(totalPages) {
            if (!elements.pagination.pageNumbers) return;
            ...
            elements.pagination.pageNumbers.innerHTML = pageNumbers;
        }

        function updatePaginationInfo(showingFrom, showingTo, total) {
            if (elements.pagination.showingFrom) {
                elements.pagination.showingFrom.textContent = showingFrom;
            }
            if (elements.pagination.showingTo) {
                elements.pagination.showingTo.textContent = showingTo;
            }
            if (elements.pagination.totalResults) {
                elements.pagination.totalResults.textContent = total;
            }
        }
        ...
        function renderHistory(histArr) {
            ...
            const items = histArr.map(h => {
                const when = formatDate(h.routed_at || h.created_at);
      ...
      `;
        ...
        async function openModalFor(id) {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    showError(`Failed to load ticket details (${res.status}). Please try again or contact support.`);
                    return;
                }
                ...
                if (!contentType || !contentType.includes('application/json')) {
                    ...
                    try {
                        ...
                        console.log('Actual response (first 500 chars):', responseText.substring(0, 500));
                    } catch (e) {
                        console.log('Could not read response as text');
                    }
                    
                    showError('Server returned an unexpected response format. Please check your network connection and try again.');
                    return;
                }
                
                const t = await res.json();
                console.log('Parsed ticket data:', t);
                
                if (!t) {
                    console.error('No ticket data received');
                    showError('No ticket data found. Please try again.');
                    return;
                }
                ...
                const createdAt = formatDate(t.date_created || t.created_at);
                const updatedAt = formatDate(t.updated_at);
        ...
        if (tmForwardApply && tmForwardSelect) {
            tmForwardApply.addEventListener('click', async () => {
                ...
                if (!tmForwardSelect.value) {
                    showWarning('Please choose a user to forward to.');
                    return;
                }
                const userId = tmForwardSelect.value;
                try {
                    const forwardUrl = `/staff/tickets/${currentTicketId}/forward`;
                    console.log('Forwarding ticket to URL:', forwardUrl);
                    ...
                    if (res.ok) {
                        ...
                        console.log('Forward success:', result);
                        showSuccess('Ticket forwarded successfully!');
                        closeModal();
                        loadTickets();
                    } else {
                        const errorText = await res.text();
                        console.error('Forward failed', res.status, errorText);
                        showError(`Failed to forward ticket (${res.status}). Please try again.`);
                    }
                } catch (err) {
                    console.error('Forward error', err);
                    showError('Network error during forward. Please check your connection.');
                }
            });
        }
        ...
        if (tmSendResponse && tmResponse) {
            tmSendResponse.addEventListener('click', async () => {
                ...
                if (!msg) {
                    showWarning('Please enter a response message.');
                    return;
                }
                if (!currentTicketId) {
                ...
                setButtonLoading(tmSendResponse, true);
                
                try {
                    const respondUrl = `/staff/tickets/${currentTicketId}/respond`;
                    console.log('Sending response to URL:', respondUrl);
                    ...
                    if (res.ok) {
                        ...
                        const errorText = await res.text();
                        console.error('Send response failed', res.status, errorText);
                        showError(`Failed to send response (${res.status}). Please check mail configuration or try again.`);
                    }
                } catch (err) {
                    console.error('Send response error', err);
                    showError('Network error while sending response. Please check your connection.');
                } finally {
                    // Remove loading state
                    setButtonLoading(tmSendResponse, false);
                }
            });
        }

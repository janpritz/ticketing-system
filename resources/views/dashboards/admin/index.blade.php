    ...
    <div id="content-wrapper" class="px-10 sm:ml-64 transition-all duration-300">
        ...
        <main class="mt-4 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="mt-2 text-3xl font-semibold text-slate-900"><span
                                    id="openTicketsCount">{{ number_format($openTickets ?? 0) }}</span></div>
                            @if (($openTicketsDelta ?? 0) > 0)
                                <div id="openTicketsDeltaWrap"
                                    ...
                                    <span id="openTicketsDelta">+{{ number_format($openTicketsDelta ?? 0) }} from
                                        yesterday</span>
                                </div>
                            @endif
                        </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="mt-2 text-3xl font-semibold text-slate-900"><span
                                    id="faqCountValue">{{ number_format($faqCount ?? 0) }}</span></div>
                            ...
                            @if (($faqNewCount ?? 0) > 0)
                                <div id="faqNewWrap" class="mt-1 text-xs text-emerald-600 flex items-center gap-1">
                                    ...
                                    <a href="{{ route('admin.faqs.index', ['filter' => 'new']) }}"
                        ...
                        <div class="rounded-md bg-blue-50 p-2 text-blue-600 border border-blue-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24"
                                fill="currentColor">
                                <path
                                    d="M12 3l9 4.5V12c0 5-4 9-9 9s-9-4-9-9V7.5L12 3zm0 2.2L5 8.1V12c0 3.9 3.1 7 7 7s7-3.1 7-7V8.1l-7-2.9z" />
                            </svg>
                        </div>
                    </div>
                </div>
                ...
                <div id="activeStaffCard"
                    class="bg-white rounded-xl border border-gray-200 p-4 cursor-pointer hover:bg-gray-50" role="button"
                    tabindex="0" aria-label="Open active staff list">
                    <div class="flex items-start justify-between">
                        <div>
                            <div class="text-xs font-medium text-slate-500">Active Staff (last 10 min)</div>
                            <div class="mt-2 text-3xl font-semibold text-slate-900"><span
                                    id="activeStaffCount">{{ number_format($activeStaffCount ?? 0) }}</span></div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="mt-2 text-3xl font-semibold text-slate-900"><span
                                    id="userCountValue">{{ number_format($userCount ?? 0) }}</span></div>
                            @if (($newUsers ?? 0) > 0)
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            ...
                            <tbody id="openListBody" class="divide-y divide-gray-100">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="py-3 pl-3 pr-5 align-top">
                                            <button type="button" data-id="{{ $t->id }}"
                                                class="btn-view inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50">
                                                View
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5"
                                                    viewBox="0 0 24 24" fill="currentColor">
                                                    <path d="M9 5l7 7-7 7" />
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-5 py-10 text-center text-sm text-gray-500">No open
                                            tickets.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            ...
                            <tbody id="inProgressListBody" class="divide-y divide-gray-100">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 py-3 align-top">
                                            ...
                                            <div class="text-xs text-gray-500">
                                                {{ optional($t->staff)->name ? 'Staff: ' . optional($t->staff)->name : '' }}
                                            </div>
                                        </td>
                                        <td class="px-3 py-3 align-top">
                                            <span
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-5 py-10 text-center text-sm text-gray-500">No tickets
                                            assigned to Primary Administrator.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div class="relative mx-auto my-6 w-[90%] max-w-3xl max-h-[90vh]">
            <div class="bg-white rounded-xl shadow-xl ring-1 ring-black/5 overflow-hidden max-h-[90vh] flex flex-col">
                <div class="flex items-center justify-between px-5 py-4 border-b">
                    ...
                    <div>
                        <button type="button" data-modal-close aria-label="Close ticket details"
                            class="inline-flex items-center justify-center h-8 w-8 rounded-md text-slate-600 hover:text-slate-800 hover:bg-gray-50">
                        ...
                        <div class="flex items-center gap-2">
                            <select id="tmRerouteSelect" class="rounded-md border-gray-300 text-sm px-3 py-2">
                                ...
                                @foreach ($staffContacts ?? [] as $staff)
                                    <option value="{{ data_get($staff, 'id') }}">{{ data_get($staff, 'name') }}</option>
                                @endforeach
                            </select>
                            <button id="tmRerouteInlineBtn" type="button"
                                class="hidden rounded-md bg-white border border-gray-200 px-3 py-1.5 text-sm">Forward</button>
                        </div>

                        <div id="tmRerouteHistoryContainer" class="mt-3 hidden">
                            <button type="button" id="tmHistoryToggle"
                                class="w-full text-left text-sm text-slate-600 px-2 py-1 rounded-md hover:bg-gray-50 flex items-center justify-between">
                                <span>Forward History</span>
                                <svg id="tmHistoryIcon" class="h-4 w-4 text-slate-500 transition-transform"
                                    viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6 9l6 6 6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                            </button>
                            <div id="tmHistoryPanel"
                                class="mt-2 px-2 py-2 border rounded-md bg-gray-50 hidden max-h-64 overflow-auto"></div>
                        </div>
                    </div>
                </div>
                <div class="px-5 py-3 border-t flex items-center justify-between gap-3">
                    <div></div>
                    <div class="flex items-center gap-2">
                        <button id="tmSendResponse" type="button" disabled
                            class="rounded-md bg-gray-300 text-white px-4 py-1.5 text-sm">Send</button>
                    </div>
                </div>
            </div>
        </div>
...
>>>>>>> Stashed changes
                ...
                tbody.innerHTML = rows.length ? rows.join('') :
                    `<tr><td colspan="4" class="px-5 py-10 text-center text-sm text-gray-500">No tickets assigned to you.</td></tr>`;
            }
            async function refreshAdminData() {
                ...
                try {
                    ...
                    if (Array.isArray(data.activeStaff)) {
                        ...
                        if (typeof asd !== 'undefined' && asd && !asd.classList.contains('hidden') &&
                            typeof renderActiveStaff === 'function') {
            ...
            // Expose refresh function to global scope so modal handlers can invoke it
            // without relying on closure scoping (used by inline Forward / Send handlers).
            try { window.refreshAdminData = refreshAdminData; } catch (_) {}
...
>>>>>>> Stashed changes
                        ...
                        if (!Array.isArray(list) || list.length === 0) {
                            ...
                            if (historyPanel) {
                                historyPanel.innerHTML = list.map(h => {
                                    ...
                                    const staffName = (h.staff && (h.staff.name || h.staff_name)) || h
                                        .staff_name || '-';
                    ...
                    // Show/hide and hook up reroute controls (inline button appears when a staff is selected)
                    try {
                        const tmRerouteSelectEl = document.getElementById('tmRerouteSelect');
                        const tmRerouteInlineBtnEl = document.getElementById('tmRerouteInlineBtn');
                        const csrfEl = document.querySelector('meta[name="csrf-token"]');
                        const csrfToken = csrfEl ? csrfEl.getAttribute('content') : '';
                        if (tmRerouteSelectEl && tmRerouteInlineBtnEl) {
                            // initial visibility
                            tmRerouteInlineBtnEl.classList.toggle('hidden', !tmRerouteSelectEl.value);
                            // handle change
                            tmRerouteSelectEl.onchange = () => tmRerouteInlineBtnEl.classList.toggle('hidden', !tmRerouteSelectEl.value);
                            // attach click handler
                            tmRerouteInlineBtnEl.onclick = async function() {
                                const staffId = tmRerouteSelectEl.value;
                                if (!staffId) {
                                    alert('Please choose a staff to forward to.');
                                    return;
                                }
                                const rUrl = "{{ url('/admin/tickets') }}/" + encodeURIComponent(id) + "/reroute";
                                try {
                                    ...
                                    if (resp && resp.ok) {
                                        ...
                                        // If dashboard has refreshAdminData function, call it to update UI
                        ...
                        console.warn('Reroute setup failed', e);
                    }
    
                    // Send response (admin dashboard) - enable/disable button and post to respond endpoint
                    try {
                        const sendBtn = document.getElementById('tmSendResponse');
                        const respEl = document.getElementById('tmResponse');
                        const csrfEl2 = document.querySelector('meta[name="csrf-token"]');
                        const csrf = csrfEl2 ? csrfEl2.getAttribute('content') : '';
                        ...
                        if (sendBtn) {
                            sendBtn.onclick = async () => {
                                ...
                                if (!msg) {
                                    if (window.Swal) Swal.fire({ position: 'top-end', icon: 'warning', toast: true, title: 'Please enter a response', showConfirmButton: false, timer: 2000 });
                                    return;
                                }
                                const rUrl = "{{ url('/admin/tickets') }}/" + encodeURIComponent(id) + "/respond";
                                try {
                                    ...
                                    if (res && res.ok) {
                                        try { localStorage.setItem('ts_tickets_changed', String(Date.now())); } catch(e){}
                                        // Refresh current tab immediately if function exists, otherwise reload as a fallback.
                                        ...
                                        if (ticketModal) ticketModal.classList.add('hidden');
                                        if (window.Swal) Swal.fire({ position: 'top-end', icon: 'success', toast: true, title: 'Response sent', showConfirmButton: false, timer: 2500, timerProgressBar: true });
                                    } else {
                                        const txt = await res.text().catch(()=>'');
                                        console.error('Send response failed', txt);
                                        if (window.Swal) Swal.fire({ position: 'top-end', icon: 'error', toast: true, title: 'Send failed', showConfirmButton: false, timer: 3000 });
                                        else alert('Send failed');
                                    }
                                } catch (err) {
                                    console.error('Send response error', err);
                                    if (window.Swal) Swal.fire({ position: 'top-end', icon: 'error', toast: true, title: 'Network error while sending response', showConfirmButton: false, timer: 3000 });
                                } finally {
                        ...
                        console.warn('Send setup failed', e);
                    }

...
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-4">
        <a href="{{ route('admin.users.index') }}" class="inline-flex items-center text-sm text-slate-600 hover:text-slate-800">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 24 24" fill="currentColor"><path d="M15 19l-7-7 7-7"/></svg>
            Back to Users
        </a>
    </div>
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Edit Staff</h1>
            <p class="text-sm text-slate-500">Update staff account details</p>
        </div>
        <div>
            <span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ring-1 ring-inset {{ $user->role === 'Primary Administrator' ? 'text-purple-700 bg-purple-50 ring-purple-600/20' : 'text-slate-700 bg-slate-50 ring-slate-600/20' }}">
                {{ $user->role }}
            </span>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="role" value="{{ old('role', $user->role) }}" required placeholder="e.g. IT Support"
                           class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />

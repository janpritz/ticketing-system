...
<div id="viewPendingFaqModal" class="fixed inset-0 z-50 hidden">
  ...
  <div class="absolute inset-0 flex items-center justify-center p-4">
    <div class="relative w-full max-w-2xl bg-white rounded-lg shadow border border-gray-200">
      ...
      <button type="button" class="absolute top-3 right-3 text-slate-500 hover:text-slate-700 pending-modal-close" aria-label="Close">
      ...
      <form id="viewPendingFaqForm" class="p-4 space-y-4" onsubmit="return false;">
        ...
        <div>
          ...
          <input type="text" id="pending_view_topic" readonly
        ...
        <div class="pt-2 flex items-center justify-between">
          <div class="text-xs text-slate-500" id="pending_view_timestamps"></div>
          <div class="flex items-center gap-3">
            <button id="chatbotTrainedBtn" type="button"
                    class="rounded-md bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium px-4 py-2">Chatbot Trained</button>
            <button id="responseTrainedBtn" type="button"
                    class="rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-4 py-2">Response Trained</button>
          </div>
        </div>
      </form>
    </div>
...
<div id="pending-faqs-state" class="hidden"
     data-list-url="{{ route('admin.faqs.pending.list') }}"
...
(function () {
  ...
  async function fetchList(page = 1) {
    ...
    try {
      ...
      faqsTbody.innerHTML = `<tr><td colspan="4" class="px-5 py-6 text-center text-sm text-red-600">Error loading FAQs</td></tr>`;
  ...
  function renderTable(items) {
    if (!items || items.length === 0) {
      faqsTbody.innerHTML = `<tr><td colspan="4" class="px-5 py-10 text-center text-sm text-gray-500">No pending FAQs.</td></tr>`;
      return;
    }
  ...
  const pendingViewTopic = document.getElementById('pending_view_topic');
  ...
  const chatbotTrainedBtn = document.getElementById('chatbotTrainedBtn');
  ...
  async function onVerifyClick(e) {
    ...
    try {
      ...
      pendingViewTimestamps.innerHTML = `<div class="text-xs">Created: ${escapeHtml(f.created_at || '')} &nbsp; Updated: ${escapeHtml(f.updated_at || '')}</div>`;
      openModal(viewModal);
    } catch (err) {
  ...
  // Chatbot-trained action
  chatbotTrainedBtn.addEventListener('click', async () => {
    const id = pendingViewFaqId.value;
    await trainFaq(id, chatbotTrainedBtn, 'Mark this FAQ as trained (Chatbot)?');
  });

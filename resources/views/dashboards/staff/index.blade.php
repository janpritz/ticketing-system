...
@section('staff-content')
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-red-50 border border-red-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-red-800">
                                You have {{ $overdueCount }} overdue ticket{{ $overdueCount > 1 ? 's' : '' }}
                            </h3>
                    ...
                    <div class="ml-4 flex-shrink-0">
                        <button type="button" id="overdueDropdownBtn" class="bg-red-100 hover:bg-red-200 text-red-800 px-3 py-2 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                            View Tickets
                        </button>
                    </div>
                </div>
                <!-- Overdue Tickets List (hidden by default) -->
                <div id="overdueTicketsList" class="mt-4 hidden">
                    <div class="space-y-2">
                        @foreach($overdueTickets as $ticket)
                            <div class="flex items-center justify-between bg-white p-3 rounded-md border border-red-200">
                                <div>
                                    <p class="text-sm font-medium text-gray-900">Ticket #{{ $ticket->id }}</p>
                                    <p class="text-sm text-gray-500">{{ Str::limit($ticket->question, 50) }}</p>
                                    <p class="text-xs text-gray-400">Created {{ $ticket->date_created->diffForHumans() }}</p>
                                </div>
                                <a href="#" data-action="view" data-id="{{ $ticket->id }}" class="btn-view text-red-600 hover:text-red-800 text-sm font-medium">View</a>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="grid grid-cols-12 gap-6">
            <!-- Main content -->
            <section class="col-span-12 space-y-6 flex flex-col">
                <!-- KPI cards -->
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6 order-1 mt-0">
                    <!-- Open -->
    ...
    <script>
        ...
        if (overdueDropdownBtn && overdueTicketsList) {
            overdueDropdownBtn.addEventListener('click', () => {
                ...
                overdueDropdownBtn.textContent = overdueTicketsList.classList.contains('hidden') ? 'View Tickets' : 'Hide Tickets';
            });
        }

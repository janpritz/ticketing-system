...
return new class extends Migration
{
     ...
     * Run the migrations.
     *
     * Replace "In-Progress" with "Forwarded" in data and constraints.
    ...
    public function up(): void
    {
        ...
        DB::statement("UPDATE tickets SET status = 'Forwarded' WHERE status IN ('In-Progress','Re-routed')");
        DB::statement("UPDATE ticket_routing_histories SET status = 'Forwarded' WHERE status IN ('In-Progress','Re-routed')");
        ...
        if ($driver === 'pgsql') {
            ...
            DB::statement("ALTER TABLE public.tickets ADD CONSTRAINT tickets_status_check CHECK (status IN ('Open','Forwarded','Closed'))");
...
SQL);
            ...
            DB::statement("ALTER TABLE public.ticket_routing_histories ADD CONSTRAINT ticket_routing_histories_status_check CHECK (status IN ('Open','Forwarded','Closed'))");
        } elseif ($driver === 'mysql') {
            // MySQL: change ENUM set
            DB::statement("ALTER TABLE `tickets` MODIFY COLUMN `status` ENUM('Open','Forwarded','Closed') NOT NULL DEFAULT 'Open'");
            DB::statement("ALTER TABLE `ticket_routing_histories` MODIFY COLUMN `status` ENUM('Open','Forwarded','Closed') NOT NULL");
        } else {
...
SQL);
        ...
        // Revert data
        DB::statement("UPDATE tickets SET status = 'In-Progress' WHERE status = 'Forwarded'");
        DB::statement("UPDATE ticket_routing_histories SET status = 'In-Progress' WHERE status = 'Forwarded'");
    }
};
